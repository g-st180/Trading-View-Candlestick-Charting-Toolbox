...
import FunctionalityMetricsComparisonTable from './FunctionalityMetricsComparisonTable';
...
export default function MasterTableModal() {
    const [isOpen, setIsOpen] = useState(false);
    const [data, setData] = useState<FunctionalityComparisonData | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [timeRange, setTimeRange] = useState<TimeRange>('24h');
    const { technicalMetricsStart, technicalMetricsEnd } = useDashboardStore();
    const [startDate, setStartDate] = useState<string>(technicalMetricsStart || '');
    const [endDate, setEndDate] = useState<string>(technicalMetricsEnd || '');
    ...
    // Persist threshold filters across modal open/close
    ...
    const fetchData = useCallback(async (startDate: string, endDate: string) => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/api/models/functionality_comparison?${params.toString()}`);
    ...
    }, [isOpen, startDate, endDate, fetchData]);

    useEffect(() => {
        const handleOpen = () => {
            setIsOpen(true);
            setStartDate(technicalMetricsStart || '');
            setEndDate(technicalMetricsEnd || '');
            if (technicalMetricsStart && technicalMetricsEnd) {
                fetchData(technicalMetricsStart, technicalMetricsEnd);
            }
        };

        window.addEventListener('openMasterTable', handleOpen);
        return () => window.removeEventListener('openMasterTable', handleOpen);
    }, [technicalMetricsStart, technicalMetricsEnd, fetchData]);
        ...
        >
            ...
            <div className="relative w-[95vw] h-[90vh] rounded-lg border border-cyan-500/30 bg-black/90 backdrop-blur-sm shadow-2xl">
                <div className="flex items-center justify-between border-b border-cyan-500/20 px-4 py-3">
                    <h2 className="metallic-title text-lg font-semibold uppercase tracking-wide">Master Table</h2>
                    <button
                ...
                {/* Date filters and time range buttons */}
                ...
                <div className="h-[calc(100%-120px)] overflow-hidden">
                    <FunctionalityMetricsComparisonTable 
                        data={data} 
                        isLoading={isLoading}
                        onModelClick={() => setIsOpen(false)}
                        startDate={startDate}
                        endDate={endDate}
                        timeRange={timeRange}
                        thresholdFilters={thresholdFilters}
                        onThresholdFiltersChange={setThresholdFilters}
                    />
                </div>
            </div>
        </div>
